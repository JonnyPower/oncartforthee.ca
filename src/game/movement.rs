...
fn handle_movement(
    ...
    mut player_q: Query<(&mut Transform, &MovementSpeed), With<Player>>,
    ...
    if xz_plane_movement != Vec2::ZERO {
        match player_q.get_single_mut() {
            Ok((mut player_t, speed)) => {
                ...
                player_t.translation = new_location;
                let mut camera_t = camera_q.single_mut();
                camera_t.translation = camera_t.translation.lerp(
                    Vec3::new(player_t.translation.x, camera_t.translation.y, player_t.translation.z),
                    time.delta_secs() * 5.0
                );
                let mut rng = rng();
                // Spawn a bunch of particles.
                for _ in 0..5 {
                    let horizontal = Vec2::new(rng.random_range(-2.0..2.0), -2.0);
                    let vertical = rng.random_range(0.0..4.0);
                    let size = rng.random_range(0.01..0.05);
                    let particle_spawn = player_t.translation - Vec3::new(0.0, 0.0, 0.8);
