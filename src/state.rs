...
impl Plugin for StatePlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(OnExit(AppState::TitleMenu), hide_title_on_state);
        app.add_systems(OnEnter(AppState::TitleMenu), show_title_on_state);
        app.add_systems(OnExit(AppState::InGame), stop_game_on_state);
        app.add_systems(OnEnter(AppState::InGame), initial_in_game_state);
...
fn stop_game_on_state(mut in_game_state: ResMut<NextState<InGameState>>) {
